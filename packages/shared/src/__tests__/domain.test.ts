...
import {
  applyIdempotentLike, canFollow, isEthereumAddress, isUsername, mapFeedPost,
  parseMentions, validatePost, validateProfile, type FeedRowLike,
} from "../index";
...
describe("wallet-address validation", () => {
  it("accepts only 0x plus 40 hex characters", () => {
    expect(isEthereumAddress("0x1111111111111111111111111111111111111111")).toBe(true);
...
describe("post and profile validation", () => {
  ...
  it("validates the complete editable profile", () => {
    expect(validateProfile({ username: "novaforms", displayName: "Nova", websiteUrl: "https://example.com" })).toEqual({});
    expect(validateProfile({ username: "Admin", displayName: "", websiteUrl: "http://example.com" })).toEqual(expect.objectContaining({ username: expect.any(String), displayName: expect.any(String), websiteUrl: expect.any(String) }));
  });
});

describe("social rules", () => {
  it("prevents self-following", () => expect(canFollow("user_a", "user_a")).toBe(false));
  it("permits following another user", () => expect(canFollow("user_a", "user_b")).toBe(true));
...
describe("mention parsing", () => {
  it("deduplicates valid lowercase mentions and ignores reserved names", () => {
    expect(parseMentions("hi @novaforms and @novaforms, cc @settings @UPPER")).toEqual(["novaforms"]);
  });
});

describe("feed mapping", () => {
  it("maps D1 integer flags to booleans", () => {
    const row = { currentUserLiked: 1, currentUserFollowsAuthor: 0 } as FeedRowLike;
